...
export function Dashboard({ onSelectModule, onOpenInstructions, onOpenExamGuide, onOpenMockExam, progress }: DashboardProps) {
  ...
  return (
    <div className="animate-fade-in">
      <div className="relative mb-8 overflow-hidden rounded-3xl bg-gradient-to-br from-teal-600 via-teal-700 to-cyan-800 px-4 py-6 shadow-xl sm:px-12 sm:py-16">
        ...
        <div className="absolute right-4 top-4 z-20" ref={dropdownRef}>
          <button
            ...
            className="flex items-center gap-2 rounded-xl border border-white/30 bg-white/20 px-3 py-2 text-xs font-semibold text-white shadow-sm backdrop-blur-md transition-all hover:bg-white/30"
          >
            ...
            <div className="absolute right-0 z-30 mt-2 max-h-80 w-52 overflow-y-auto rounded-2xl border border-slate-100 bg-white py-2 shadow-xl animate-fade-in">
              ...
              {languages.map((l) => (
                <button
                  ...
                  className={cn(
                    'flex w-full items-center justify-between px-3.5 py-2 text-left text-xs font-medium transition-colors',
                    lang === l.id ? 'bg-teal-50 font-semibold text-teal-800' : 'text-slate-700 hover:bg-slate-50'
                  )}
                >
        ...
        <div className="relative flex flex-col items-center justify-between gap-6 pt-12 text-center sm:flex-row sm:items-start sm:pt-0 sm:text-left">
          <div className="order-1 flex shrink-0 flex-col items-center sm:order-2">
            <img
              ...
              className="h-44 w-44 rounded-2xl object-contain object-center drop-shadow-2xl sm:h-48 sm:w-48"
            />
            <div className="mt-2.5 text-center">
              <p className="text-sm font-semibold text-white/90">{t.assistant}</p>
              <p className="text-xs text-white/75">{t.assistantSub}</p>
            </div>
          </div>
        ...
        {[
          ...
          return (
            <button
              ...
              className={cn(
                'group relative overflow-hidden rounded-2xl border-2 bg-white p-6 text-left transition-all duration-300 hover:-translate-y-1 hover:shadow-lg animate-slide-up',
                mod.border
              )}
              style={{ animationDelay: `${idx * 80}ms` }}
            >
              <div className="mb-4 flex items-start gap-4">
                <div className={cn('flex h-14 w-14 shrink-0 items-center justify-center rounded-xl transition-transform group-hover:scale-110', mod.bg)}>
                  <Icon className={cn('h-7 w-7', mod.color)} />
                </div>
                <div className="min-w-0 flex-1">
                  <h3 className="text-xl font-bold text-slate-900">{mod.title}</h3>
                  <p className="text-sm font-medium text-slate-500">{mod.subtitle}</p>
                </div>
              ...
              {p && (
                <div className="mb-2 flex justify-end">
                  <span className="text-xs font-medium text-slate-400">{t.best}: {p.bestScore}% · {t.attempts}: {p.attempts}</span>
                </div>
              )}
      ...
      <div className="mt-8 grid grid-cols-1 gap-4 sm:grid-cols-3">
        <button
          ...
          className="group flex items-center gap-4 rounded-2xl border-2 border-slate-200 bg-white p-5 text-left transition-all duration-300 hover:-translate-y-0.5 hover:shadow-lg animate-slide-up"
        >
