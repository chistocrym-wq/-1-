...
type Screen = 'home' | 'teil1' | 'teil2' | 'teil3';
  ...
  if (screen === 'home') {
    return (
      <div className="animate-fade-in">
        <Header onBack={onBack} subtitle="Устная часть · тренировка по формату A1" />

        <div className="mb-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <p className="text-[17px] leading-7 text-slate-700">
            Здесь тренируются все три части Sprechen: короткое представление, вопросы по карточкам и просьбы по картинкам.
            Отвечайте вслух — запись можно прослушать и при подключённом API проверить у Otto.
          </p>
        </div>
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          <PartCard
            ...
            description="Представьтесь по опорным словам и потренируйте имя и номер."
            meta="1 экзаменационный шаблон"
            onClick={() => openPart(1)}
          />
          <PartCard
            number={2}
            title="Informationen"
            description="Получите тему и слово. Задайте партнёру простой вопрос."
            meta={`${speakingTeil2Cards.length} карточек`}
            onClick={() => openPart(2)}
          />
          <PartCard
            number={3}
            title="Bitten"
            description="Посмотрите на картинку, сформулируйте просьбу и реакцию."
            meta={`${speakingTeil3Cards.length} карточек`}
            onClick={() => openPart(3)}
          />
        </div>
      </div>
    ...
    return (
      <div className="animate-fade-in">
        ...
        <div className="mb-5 overflow-hidden rounded-2xl border border-slate-300 bg-white shadow-sm">
          ...
          <div className="divide-y divide-slate-200 px-6 py-2 text-center">
            {speakingTeil1.keywords.map((keyword) => (
              <div key={keyword} className="py-3 text-3xl font-bold tracking-tight text-slate-900">
                {keyword}
              </div>
            ))}
          </div>
        ...
        <div className="mb-5 rounded-2xl border border-slate-200 bg-white p-5">
          ...
          <div className="space-y-2">
            {speakingTeil1.followUps.map((item) => (
              <div key={item} className="rounded-xl bg-slate-50 px-4 py-3 text-[16px] text-slate-700">
                {item}
              </div>
            ))}
          </div>
        ...
        <VoiceRecorder
          ...
          evaluation={{
            mode: 'teil1',
            expectedPoints: speakingTeil1.checkPoints,
          }}
        ...
        <SampleBox
          show={showSample}
          onToggle={() => setShowSample((value) => !value)}
          title="Пример короткого ответа"
        >
    ...
    return (
      <div className="animate-fade-in">
        ...
        <SampleBox
          show={showSample}
          onToggle={() => setShowSample((value) => !value)}
          title="Один из возможных вариантов"
        >
        ...
        <BottomActions
          disabled={!practiced}
          onNext={() => {
            setTeil2Index((value) => (value + 1) % speakingTeil2Cards.length);
            setShowSample(false);
            setPracticed(false);
          }}
          onShuffle={() => {
            setTeil2Index((value) => nextRandomIndex(value, speakingTeil2Cards.length));
            setShowSample(false);
            setPracticed(false);
          }}
    ...
    <div className="animate-fade-in">
      ...
      <div className="mx-auto mb-5 max-w-xl overflow-hidden rounded-2xl border border-slate-400 bg-white shadow-sm">
        ...
        <div className="p-5 sm:p-7">
          <SpeakingCardIllustration visual={card.visual} alt={card.alt} />
        </div>
      </div>
      ...
      <VoiceRecorder
        ...
        evaluation={{
          mode: 'teil3',
          object: card.alt,
          sampleRequest: card.sampleRequest,
        }}
        onPracticed={() => setPracticed(true)}
      />
      ...
      <SampleBox
        show={showSample}
        onToggle={() => setShowSample((value) => !value)}
        title="Пример просьбы и реакции"
      >
      ...
      <BottomActions
        disabled={!practiced}
        onNext={() => {
          setTeil3Index((value) => (value + 1) % speakingTeil3Cards.length);
          setShowSample(false);
          setPracticed(false);
        }}
        onShuffle={() => {
          setTeil3Index((value) => nextRandomIndex(value, speakingTeil3Cards.length));
          setShowSample(false);
          setPracticed(false);
        }}
        nextLabel="Следующая карточка"
...
function PartCard({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="group min-h-[220px] rounded-2xl border border-slate-200 bg-white p-5 text-left shadow-sm transition hover:-translate-y-0.5 hover:border-slate-300 hover:shadow-md"
    >
      ...
      <div className="text-lg font-bold text-slate-950">Teil {number} · {title}</div>
...
function InstructionBox({ german, russian }: { german: string; russian: string }) {
  return (
    <div className="mb-5 rounded-2xl border border-slate-200 bg-slate-50 p-5">
      ...
      <p className="mt-2 text-sm leading-6 text-slate-500">{russian}</p>
    </div>
  );
}
...
function ExamCardHeader({ label }: { label: string }) {
  return (
    <div className="flex items-center justify-between bg-slate-700 px-4 py-2 text-xs font-bold uppercase tracking-wide text-white">
      <span>Start Deutsch 1 · Training</span>
      <span>{label}</span>
    </div>
  );
}
  ...
  return (
    <div className="mb-5 rounded-2xl border border-slate-200 bg-white p-5">
      <button
        type="button"
        onClick={onToggle}
        className="inline-flex min-h-0 items-center gap-2 text-sm font-semibold text-slate-700 hover:text-slate-950"
      >
        <Eye className="h-4 w-4" />
        {show ? 'Скрыть пример' : 'Показать пример после своего ответа'}
      </button>
      {show && (
        <div className="mt-4 rounded-xl bg-amber-50 p-4 text-[16px] leading-7 text-slate-700">
          <div className="mb-2 text-xs font-bold uppercase tracking-wide text-amber-700">{title}</div>
          {children}
        </div>
      )}
    </div>
    ...
    <div className="flex flex-col-reverse gap-3 pb-8 sm:flex-row sm:justify-end">
      {onShuffle && (
        <button
          type="button"
          onClick={onShuffle}
          className="inline-flex min-h-[52px] items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-5 py-3 font-semibold text-slate-700 hover:bg-slate-50"
        >
      ...
      <button
        ...
        disabled={disabled}
        className={cn(
          'inline-flex min-h-[52px] items-center justify-center gap-2 rounded-xl px-6 py-3 font-semibold transition',
          disabled
...
function VoiceRecorder({
  ...
  const startRecording = useCallback(async () => {
    ...
    try {
      if (!navigator.mediaDevices?.getUserMedia || typeof MediaRecorder === 'undefined') {
        throw new Error('Браузер не поддерживает запись через микрофон.');
      }
      ...
      stopTracks();
      const message = recordingError instanceof Error ? recordingError.message : 'Не удалось включить микрофон.';
      setError(message);
    }
  ...
  const checkWithOtto = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(payload?.error || 'Проверка Otto сейчас недоступна. Запись можно прослушать и продолжить тренировку.');
      }
      setResult(payload as EvaluationResult);
    } catch (checkingError) {
      const message = checkingError instanceof Error ? checkingError.message : 'Не удалось проверить запись.';
      setError(message);
    } finally {
      setChecking(false);
    }
    ...
    <div className="mb-5 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="mb-4 flex items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm leading-6 text-slate-500">
            Нажмите микрофон, скажите ответ и остановите запись. Не нужно говорить медленно или по слогам.
          </p>
        </div>
        <div className="rounded-lg bg-slate-50 px-3 py-2 text-sm font-bold tabular-nums text-slate-700">
          {formatTime(elapsed)}
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3">
        <button
          ...
          className={cn(
            ...
            isRecording
              ...
              : 'border-2 border-slate-300 bg-white text-slate-900 hover:border-slate-500'
          )}
        >
        ...
        {audioUrl && !isRecording && (
          <>
            <audio src={audioUrl} controls className="h-11 min-w-[210px] max-w-full flex-1" />
            <button
              type="button"
              onClick={resetRecording}
              className="inline-flex min-h-[44px] items-center gap-2 rounded-xl border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-600 hover:bg-slate-50"
            >
              <RefreshCw className="h-4 w-4" />
              Перезаписать
            </button>
      ...
      {audioBlob && !isRecording && (
        <div className="mt-4 border-t border-slate-100 pt-4">
          <button
            ...
            disabled={checking}
            className="inline-flex min-h-[46px] items-center gap-2 rounded-xl bg-amber-100 px-4 py-2 text-sm font-bold text-amber-900 transition hover:bg-amber-200 disabled:cursor-wait disabled:opacity-60"
          >
            <Sparkles className="h-4 w-4" />
            {checking ? 'Otto слушает…' : 'Проверить с Otto'}
          </button>
          <p className="mt-2 text-xs leading-5 text-slate-400">
            Проверяется выполнение задания и понятность ответа A1. Точная фонетическая оценка произношения не заявляется.
          </p>
        </div>
      )}
        ...
        <div className="mt-4 rounded-xl border border-emerald-200 bg-emerald-50 p-4">
          ...
          {result.transcript && (
            <div className="mt-3 rounded-lg bg-white/70 p-3 text-sm text-slate-600">
              <span className="font-semibold">Распознано:</span> {result.transcript}
            </div>
          )}
          {result.missing && result.missing.length > 0 && (
            <p className="mt-2 text-sm text-slate-600">Не прозвучало: {result.missing.join(', ')}</p>
          )}
        </div>
      )}
      ...
      {!audioBlob && !isRecording && (
        <div className="mt-4 flex items-center gap-2 text-sm text-slate-400">
          ...
          Ваша запись останется только в текущем окне до отправки на проверку.
        </div>
      )}
    </div>
  );
