...
export function ReadingModule({onBack,onComplete}:Props){
  ...
  const check=()=>{if(!current||selected===null)return;setChecked(true);if(selected===current.correct)setScore(v=>v+1)};
  ...
  if(!part)return <Home onBack={onBack} onOpen={open}/>;
  ...
  return <div className="animate-fade-in pb-8">
    <div className="mb-4 flex items-start gap-3">
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="mt-1 text-sm text-slate-500">Test {test} von 50 · Aufgabe {index+1} von {tasks.length}</p>
    ...
    <TestPicker current={test} onChange={choose}/>
...
function Home({onBack,onOpen}:{onBack:()=>void;onOpen:(p:Part)=>void}){return <div className="animate-fade-in pb-8">
...
function makeTest(part:Part,test:number):Task[]{const pool:Task[]=[];for(const set of lesenExamSets){if(part===1)for(const block of set.teil1)for(const st of block.statements)pool.push({kind:'teil1',text:block.text,sourceTitle:block.title,statement:st.statement,correct:st.correct,explanation:st.explanation});if(part===2)for(const t of set.teil2)pool.push({kind:'teil2',situation:t.situation,a:t.a,b:t.b,correct:t.correct,explanation:t.explanation});if(part===3)for(const t of set.teil3)pool.push({kind:'teil3',place:t.place,heading:t.heading,text:t.text,statement:t.statement,correct:t.correct,explanation:t.explanation})}if(!pool.length)return[];const count=Math.min(5,pool.length);const start=((test-1)*3+(part-1)*7)%pool.length;return Array.from({length:count},(_,i)=>pool[(start+i*7)%pool.length])}
